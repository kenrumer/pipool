...
MSG_LENGTH = 14
LOG_FILE = "/var/log/pipool/command_server_error.log"
...
def isControlValid(control):
  if len(control) != MSG_LENGTH:
    logger("*** isControlValid[1]: Control format invalid " + control)
    return False
  ...
  if control[1] != '@' and control[1] != '*' and control[1] != '#' and control[1] != '&':
    logger("*** isControlValid[2]: Control format invalid " + control + " [1]:" + control[0] + "[12]:" + control[12])
    return False
  if control[1] != control[12]:
    logger("*** isControlValid[3]: Control format invalid " + control + " [1]:" + control[0] + "[12]:" + control[12])
    return False
  return True
...
try:
  ...
  logger("Failed to start command_server.  Verify connection with Arduino.")
  exit(1)
